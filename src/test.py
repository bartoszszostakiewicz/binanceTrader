...
class BinanceTrader:
    ...
    def __init__(self):
        ...
        # Prices reduced by 0.4%
        self.prices_after = {symbol: price * 0.996 for symbol, price in self.prices_before.items()}
    ...
    def get_current_price(self, symbol,after = False):
        """Pobiera aktualną cenę z zdefiniowanych słowników."""
        if after:
            return self.prices_after.get(symbol, 0.0)
        return self.prices_before.get(symbol, 0.0)  # Default to 0 if symbol not found
        ...
        for symbol, balance in portfolio.items():
            if symbol != "USDT":
                ...
                if cypto_amount > 0.00:
                    ...
                    self.buy_orders_set[symbol] = (self.prices_after[symbol], cypto_amount)
    ...
    def execute_buy_orders(self):
        ...
        for symbol, (buy_price, quantity) in list(self.buy_orders_set.items()):
            if self.i % 10 == 0:
                current_price = self.get_current_price(symbol,after=True)
            else:
                current_price = self.get_current_price(symbol)
